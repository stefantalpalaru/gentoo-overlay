...
inherit cuda cuda-extra java-pkg-opt-2 cmake-multilib python-r1 virtualx xdg-utils
...
PATCHES=(
	...
	"${FILESDIR}"/opencv-4.1.2-opencl-license.patch
	"${FILESDIR}"/opencv-4.4.0-disable-native-cpuflag-detect.patch
	"${FILESDIR}"/opencv-4.13.0-use-system-flatbuffers.patch
	"${FILESDIR}"/opencv-4.8.1-use-system-opencl.patch
	"${FILESDIR}"/opencv-4.13.0-cmake-cleanup.patch
	"${FILESDIR}"/opencv-4.10.0-dnn-explicitly-include-abseil-cpp.patch
	"${FILESDIR}"/opencv-4.10.0-tbb-detection.patch
	"${FILESDIR}"/opencv-4.11.0-cmake-CMP0177.patch
	"${FILESDIR}"/opencv-5.0.0-caffe-protobuf.patch
)
...
multilib_src_configure() {
	...
	# ===================================================
		...
		-DCMAKE_INSTALL_LIBDIR="/usr/$(get_libdir)/opencv5"
	# ===================================================
...
multilib_src_install() {
	if use abi_x86_64 && use abi_x86_32; then
		MULTILIB_WRAPPED_HEADERS=( # {{{
			...
			/usr/include/opencv5/opencv2/mcc.hpp
			/usr/include/opencv5/opencv2/mcc/ccm.hpp
			/usr/include/opencv5/opencv2/mcc/checker_detector.hpp
			/usr/include/opencv5/opencv2/mcc/checker_model.hpp
	...
	for plugin in "${ED}/usr/$(get_libdir)/opencv5/libopencv_"*".$(ver_rs 1-2 '' "$(ver_cut 1-2)").${ARCH}"* ; do
		patchelf --set-soname "$(basename "${plugin}" ".$(get_libname)")" "${plugin}" || die
	done

	mv "${ED}/usr/$(get_libdir)/opencv5/"{cmake,pkgconfig} "${ED}/usr/$(get_libdir)/" || die
}
