...
IUSE="acpi compat +driver gtk3 kernel_linux +kms multilib static-libs +tools uvm wayland +X"
REQUIRED_USE="
	tools? ( X )
	static-libs? ( tools )
"
...
src_prepare() {
	...
	if use kernel_linux; then
		if kernel_is ge 5 1; then
			eapply "${FILESDIR}"/kernel-5.1.patch
		fi
		if kernel_is ge 5 2; then
			eapply "${FILESDIR}"/kernel-5.2.patch
		fi
	fi
