...
inherit bash-completion-r1 cmake qmake-utils
...
src_prepare() {
	...
	if use gimp || use gui ; then
		...
		# translation files are not automatically regenerated by the build system
		local PATH="${PATH}:$(qt5_get_bindir)"
		emake -C ../${GMIC_QT_DIR}/translations
		emake -C ../${GMIC_QT_DIR}/translations/filters
	fi
}
