...
PATCHES=(
	"${FILESDIR}"/warzone2100-4.6.1-clang-fmt-stdlib-include.patch
	"${FILESDIR}"/warzone2100-4.7.0-vulkan.patch
	"${FILESDIR}"/warzone2100-4.7.0-basis.patch
)
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DWZ_SKIP_ELF_SEPARATE_DEBUG=$(usex !debug)
...
src_install() {
	...
	gunzip "${ED}"/usr/share/man/man6/warzone2100.6.gz
}
