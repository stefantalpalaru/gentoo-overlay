...
SLOT="0"
KEYWORDS="~amd64"
IUSE="3d doc examples +georeferencer grass hdf5 mapserver netcdf opencl oracle pdal postgres python qml serial test webengine"
...
COMMON_DEPEND="
	...
	>=x11-libs/qwt-6.2.0:6=[polar(+),qt6(+),svg(+)]
	3d? ( dev-qt/qt3d:6 )
	...
	python? (
		...
		$(python_gen_cond_dep '
			...
			dev-python/pyqt6[designer,gui,multimedia,network,positioning,printsupport,sql,svg,widgets,${PYTHON_USEDEP}]
			dev-python/python-dateutil[${PYTHON_USEDEP}]
			dev-python/pytz[${PYTHON_USEDEP}]
			dev-python/pyyaml[${PYTHON_USEDEP}]
			dev-python/requests[${PYTHON_USEDEP}]
			>=dev-python/qscintilla-2.10.1[qt6,${PYTHON_USEDEP}]
	...
	serial? (
		...
		python? (
			$(python_gen_cond_dep '
				dev-python/pyqt6[serialport,${PYTHON_USEDEP}]
			')
		)
	)
...
src_prepare() {
	...
	if ! use serial; then
		sed -i \
			...
			python/PyQt6/core/core.sip.in || die
	fi
...
src_configure() {
	local mycmakeargs=(
		...
		-DQGIS_QML_SUBDIR=$(get_libdir)/qt6/qml
		-DPEDANTIC=OFF
		-DUSE_CCACHE=OFF
