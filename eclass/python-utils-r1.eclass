...
_python_set_impls() {
	...
	if [[ ! ${PYTHON_COMPAT_NO_STRICT} ]]; then
		for i in "${PYTHON_COMPAT[@]}"; do
			...
			case ${i} in
				pypy3_11|python2_7|tauthon2_8|python3_9|python3_1[1-4]|python3_1[3-4]t)
					;;
				jython2_7|pypy|pypy1_[89]|pypy2_0|pypy3|python2_[5-6]|python3_[1-9]|python3_10)
					obsolete+=( "${i}" )
					;;
				*)
...
_python_wrapper_setup() {
	...
	if [[ ! -x ${workdir}/bin/python ]]; then
		...
		if [[ ${EPYTHON} == python* || ${EPYTHON} == tauthon* ]]; then
			...
			# Python 3.8+.
			if [[ ${EPYTHON} != python[23].[67] && ${EPYTHON} != tauthon* ]]; then
